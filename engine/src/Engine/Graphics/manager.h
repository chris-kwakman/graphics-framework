...
namespace Graphics {
	...
	public:
		...
		struct shader_info
		{
			...
			fs::file_time_type m_last_time_written; 
	...
	public:
		...
		std::vector<shader_handle>	LoadShaders(std::vector<fs::path> _shader_paths);
		shader_program_handle		LoadShaderProgram(std::string _program_name, std::vector<shader_handle> _shader_handles);
		shader_program_handle		LoadShaderProgram(std::string _program_name, std::vector<fs::path> _shader_filepaths);
	...
	private:
		...
		static bool				load_shader_data(fs::path const& _path, ResourceManager::shader_data** _out_data_ptr);
