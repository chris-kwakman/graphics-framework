...
namespace Graphics {
	...
	ResourceManager::gltf_model_data const * ResourceManager::ImportModel_GLTF(const char* _filepath)
	{
		...
		*/
		...
		std::string const model_name = fs::path(_filepath).stem().string();
	...
	*/
	...
	std::vector<ResourceManager::shader_handle> ResourceManager::LoadShaders(std::vector<fs::path> _shader_paths)
	{
	...
	shader_program_handle ResourceManager::LoadShaderProgram(std::string _program_name, std::vector<fs::path> _shader_filepaths)
	{
	...
	void ResourceManager::RefreshShaders()
	{
		...
		{
			fs::path const & shader_file_path(iter->first);
			fs::file_time_type time = fs::last_write_time(shader_file_path);
